...
const EditarServicio = ({ id, actualizarServicios }) => {
  ...
  const onSubmit = (servicioViejo) => {
    if (!servicioViejo.password || servicioViejo.password.trim() === "") {
      delete servicioViejo.password;
    }
    editarServicio(servicioViejo, id).then((respuesta) => {
      if (respuesta && respuesta.status === 200) {
        Swal.fire(
          "Servicio editado",
          `El servicio ${servicioViejo.nombreServicio} fue editado correctamente`,
          "success"
        );
        reset();
        actualizarServicios();
        ...
        Swal.fire(
          "Ocurrio un error",
          `El servicio ${servicioViejo.nombreServicio} no pudo ser editado, intente en unos minutos`,
          "error"
        );
      }
    });
