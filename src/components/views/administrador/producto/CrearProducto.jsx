...
import { crearProducto } from "../../../helpers/queriesProductos";
import Swal from "sweetalert2";
import { useState } from "react";

const CrearProducto = ({actualizarProductos,handleClose}) => {
  ...
  const onSubmit = (nuevoProducto) => {
    crearProducto(nuevoProducto).then((respuesta) => {
      if (respuesta && respuesta.status === 201) {
        Swal.fire(
          "Producto creado",
          `El producto ${nuevoProducto.nombreProducto} se creo correctamente`,
          "success"
        );
        actualizarProductos()
        ...
        Swal.fire(
          "Ocurrio un error",
          `El servicio ${nuevoProducto.nombreProducto} no pudo ser creado, intente en unos minutos`,
          "error"
        );
      }
    });
